...
```
┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐
│   CloudFront    │    │   API Gateway   │    │   Lambda UI     │
│   (Frontend)    │───▶│   (REST API)    │───▶│   Handler       │
└─────────────────┘    └─────────────────┘    └─────────────────┘
                                                       │
┌─────────────────┐    ┌─────────────────┐           │
...
│   Scenario Gen  │    │   Analyzer      │    │   Deployer      │
└─────────────────┘    └─────────────────┘    └─────────────────┘
```
...
│   ├── lib/
│   │   ├── constructs/
│   │   │   ├── stepfunction-scenario-gen.ts
│   │   │   ├── api-gateway.ts     # 🆕 API Gateway構成
│   │   │   └── frontend-hosting.ts # 🆕 静的ホスティング
│   │   └── chaos-100-stack.ts
│   └── package.json
├── src/
...
│   ├── scenario-analyzer/
│   ├── deployer/
│   └── ui-handler/               # 🆕 UI用API Lambda
├── scripts/
│   └── build-and-deploy.sh       # 🆕 自動デプロイスクリプト
└── README.md
```
